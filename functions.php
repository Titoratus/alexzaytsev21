...
function my_more_options() {
	...
	add_settings_field('agent_inst', 'Instagram агента', 'display_agent_inst', 'general');
	register_setting('general', 'agent_inst');	
}
...
function display_agent_inst() {
	echo "<input type='text' name='agent_inst' autocomplete='off' value='".esc_attr(get_option('agent_inst'))."'>";
}
...
function custom_property_column( $column, $post_id ) {
    switch ( $column ) {
        ...
        case 'prop_img' :
            echo "<img src='".wp_get_attachment_url( get_post_meta( $post_id, 'prop_img', true ) )."' width='200'>";
            break;
...
if( !wp_next_scheduled('getprops_hook') )
	wp_schedule_event( time(), 'daily', 'getprops_hook' );
 
add_action( 'getprops_hook', 'getprops_send', 10, 3 );
 
function getprops_send() {
